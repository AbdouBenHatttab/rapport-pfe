...
\indent Sprint 3 covers the administrative-request workflow of Release 2. It includes document requests, loan requests, loan meetings, authorization requests, request tracking, and the related HR processing rules.
...
\indent Each request keeps a trace through its detail view, history records, status updates, and request-event notifications. Employee pages show personal request progress, and HR pages support local search and filtering.
...
\indent The analysis section groups the request concepts, decision traces, and final-file handling used in Sprint 3.
...
\indent Sprint 3 was reviewed and validated as the administrative request workflow of the platform. The delivered features allow employees to submit and track document, loan, and authorization requests, while HR managers can review, approve, reject, schedule loan meetings, upload final files when needed, and follow request history. The sprint improved traceability through clear request statuses, decision notes, history records, and notification events. Document final-file delivery and loan final-file delivery were also modeled and integrated into the workflow.
...
&
Some large sequence diagrams still need better print readability.
\\
\hline
Request history, decision notes, statuses, and notifications improved workflow traceability.
&
Final-file workflows are dense and should be visually polished before the final report pass.
\\
\hline
Splitting frontend and backend diagrams made the technical design easier to explain.
&
UI screenshots are still placeholders and must be replaced after the interface validation.
\\
\hline
\end{tabular}
